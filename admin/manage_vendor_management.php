...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($con,"select * from admin_users where name='$name'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			$update_sql="update admin_users set name='$name',password='$password',email='$email',mobile='$mobile' where id='$id'";
			mysqli_query($con,$update_sql);
		}else{
			mysqli_query($con,"insert into admin_users(name,password,email,role,status) values('$name','$password','$email',1,1)");
		}
		header('location:vendor_management.php');
		die();
	}
