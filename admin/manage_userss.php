...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			mysqli_query($con,"update users set username='$username', email='$email', password='$password' where id='$id'");
		}else{
			mysqli_query($con,"insert into users(username,password,email,status) values('$username','$password','$email','1')");
		}
		header('location:userss.php');
		die();
	}
